...
class HumanPainter extends CustomPainter {
  ...
  final double legGapFactor; // fraction of width used as gap between legs

  HumanPainter({required this.fillColor, this.legGapFactor = 0.08}); // <-- default أقل
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Rect torso = Rect.fromLTWH(w * 0.06, torsoTop, w * 0.88, torsoH); // شوي أضيق من قبل
    canvas.drawRRect(RRect.fromRectAndRadius(torso, Radius.circular(w * 0.12)), fill);

    // pelvisY: نقطة ربط الأرجل
    final double pelvisY = torso.bottom - (torsoH * 0.06);

    // -------- ARMS (simple shapes attached to torso) --------
    final Path leftArm = Path();
    leftArm.moveTo(torso.left + w * 0.02, torso.top + h * 0.02);
    leftArm.quadraticBezierTo(torso.left - w * 0.08, torso.top + h * 0.16, torso.left + w * 0.04, torso.bottom + h * 0.02);
    leftArm.lineTo(torso.left + w * 0.14, torso.bottom + h * 0.005);
    leftArm.quadraticBezierTo(torso.left + w * 0.05, torso.center.dy, torso.left + w * 0.02, torso.top + h * 0.02);
    leftArm.close();
    canvas.drawPath(leftArm, fill);

    final Path rightArm = Path();
    rightArm.moveTo(torso.right - w * 0.02, torso.top + h * 0.02);
    rightArm.quadraticBezierTo(torso.right + w * 0.08, torso.top + h * 0.16, torso.right - w * 0.04, torso.bottom + h * 0.02);
    rightArm.lineTo(torso.right - w * 0.14, torso.bottom + h * 0.005);
    rightArm.quadraticBezierTo(torso.right - w * 0.05, torso.center.dy, torso.right - w * 0.02, torso.top + h * 0.02);
    rightArm.close();
    canvas.drawPath(rightArm, fill);

    // -------- LEGS (narrower) --------
    final double legGap = w * legGapFactor;
    final double legW = (w - legGap) / 2 * 0.55; // اضيف *0.9 لجعلهم أنحف قليلاً
    ...
    final double leftFootLeft = leftLegLeft + (legW - leftFootW) / 2 - legW * 0.01;
    final double rightFootLeft = rightLegLeft + (legW - rightFootW) / 2 + legW * 0.01;
    final double footTop = pelvisY + legBodyHeight;
    final Rect leftFootRect = Rect.fromLTWH(leftFootLeft, footTop, leftFootW, footHeight);
    final Rect rightFootRect = Rect.fromLTWH(rightFootLeft, footTop, rightFootW, footHeight);
    canvas.drawOval(leftFootRect, fill);
    canvas.drawOval(rightFootRect, fill);

    // لمسة بسيطة أعلى الصدر
    final Paint accent = Paint()
      ..color = fillColor.withOpacity(0.035)
      ..style = PaintingStyle.stroke
      ..strokeWidth = 1.0;
    canvas.drawLine(Offset(torso.left + 6, torso.top + 6), Offset(torso.right - 6, torso.top + 6), accent);
  }

  @override
  bool shouldRepaint(covariant HumanPainter oldDelegate) {
    return oldDelegate.fillColor != fillColor || oldDelegate.legGapFactor != legGapFactor;
  }
}
